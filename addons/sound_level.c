...
static void update(srv_t *state) {
    ...
    int isample = adc_read_pin(state->pinM) << 6;
    if (isample > 0xffff)
        isample = 0xffff;
...
void sound_level_process(srv_t *state) {
    maybe_init(state);
...
void sound_level_handle_packet(srv_t *state, jd_packet_t *pkt) {
    sensor_handle_packet_simple(state, pkt, &state->sample, sizeof(state->sample));
}

SRV_DEF(sound_level, JD_SERVICE_CLASS_SOUND_LEVEL);

void sound_level_init(uint8_t pinL, uint8_t pinM, uint8_t pinH) {
    SRV_ALLOC(sound_level);
    state->pinL = pinL;
    state->pinM = pinM;
    state->pinH = pinH;
}
