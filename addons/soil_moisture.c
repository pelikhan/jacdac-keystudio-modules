...
static void update(srv_t *state) {
    ...
    int isample = adc_read_pin(state->pinM) * 24;
    if (isample > 0xffff)
        isample = 0xffff;
...
void soil_moisture_process(srv_t *state) {
    maybe_init(state);
...
void soil_moisture_handle_packet(srv_t *state, jd_packet_t *pkt) {
    sensor_handle_packet_simple(state, pkt, &state->sample, sizeof(state->sample));
}

SRV_DEF(soil_moisture, JD_SERVICE_CLASS_SOIL_MOISTURE);

void soil_moisture_init(uint8_t pinL, uint8_t pinM, uint8_t pinH) {
    SRV_ALLOC(soil_moisture);
    state->pinL = pinL;
    state->pinM = pinM;
    state->pinH = pinH;
    // stream every 1000ms
